...
**Evidencia de cierre:** factoría central en `apps/mobile/src/lib/query-keys.ts`; resolución y purga de scopes en `apps/mobile/src/providers/TenantScopeProvider.tsx`; limpieza atómica de transiciones en `apps/mobile/src/lib/tenant-transition.ts` y `apps/mobile/src/providers/AuthProvider.tsx`; todas las queries mobile migradas a claves de cuenta o tenant con identidad explícita. Verificado con 43 tests, typecheck y lint aislado de la infraestructura nueva.
...
**Prueba Android del 20 de agosto de 2026:** desde Clientes se seleccionó un contacto, se eligió la exportación de **Datos mínimos** y se abrió correctamente la hoja nativa de compartir. El destino Files abrió el CSV correctamente; el archivo de prueba se eliminó después. Drive recibió el archivo en su selector y la subida fue cancelada antes de confirmar; Excel lo abrió para importar y se cerró sin guardar cambios; Correo abrió un borrador con el CSV adjunto, que se canceló sin enviar. La matriz Android queda verificada; falta la pasada equivalente en iOS.
...
**Prueba Android del 20 de agosto de 2026:** el flujo autenticado completó dashboard, encuesta de bienvenida, Agenda (Día, Semana y Completo), Clientes, Caja y Wallet en un dispositivo físico. La Agenda se revalidó tras desplegar el contrato `from/to`; no se observaron errores en sus ventanas de prueba ni al reanudar desde segundo plano.
