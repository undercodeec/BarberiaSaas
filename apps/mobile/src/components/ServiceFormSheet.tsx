...
const AGENDA_COLORS = [
  ...
  '#4F46E5', '#6366F1', '#8B5CF6', '#A855F7', '#C026D3', '#DB2777',
  '#E11D48', '#F43F5E', '#7F1D1D', '#9A3412', '#92400E', '#854D0E',
  '#3F6212', '#166534', '#065F46', '#115E59', '#155E75', '#075985',
  '#2464E8', '#1E40AF', '#3730A3', '#5B21B6', '#6B21A8', '#86198F',
  '#9D174D', '#BE123C', '#475569', '#111827',
] as const;
...
export function ServiceFormSheet({
  ...
  return (
    ...
    >
          ...
          <SafeAreaView edges={['bottom']} style={styles.sheet}>
            ...
            >
              ...
              <View style={styles.fieldRow}>
                ...
                <View style={styles.rowField}>
                  <Text style={styles.label}>
                    Precio {priceType === 'fixed' || priceType === 'from' ? <Text style={styles.required}>*</Text> : null}
                  </Text>
                  <View
                    style={[
                      styles.inputShell,
                  ...
                  >
                    ...
                    <TextInput
                      ...
                      placeholder={priceType === 'fixed' || priceType === 'from' ? '15.00' : 'No aplica'}
                      placeholderTextColor="#98a0ab"
                      style={styles.input}
                      value={price}
                    />
              ...
              <View style={styles.field}>
                ...
                {priceMenuOpen ? <View style={styles.optionMenu}>{(Object.keys(PRICE_TYPES) as PriceType[]).map((option) => <Pressable key={option} onPress={() => { setPriceType(option); setPriceMenuOpen(false); }} style={styles.option}><Text style={styles.optionText}>{PRICE_TYPES[option]}</Text>{priceType === option ? <Ionicons color="#101c2d" name="checkmark" size={20} /> : null}</Pressable>)}</View> : null}
              </View>

              <View style={styles.field}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Categoría de servicio</Text>
                  <Pressable accessibilityLabel="Información sobre categorías" onPress={() => setCategoryModalOpen(true)} style={styles.infoButton}><Ionicons color="#101c2d" name="information" size={18} /></Pressable>
                </View>
              ...
              <View style={styles.field}>
                <View style={styles.labelRow}>
                  ...
                  <Pressable accessibilityLabel="Información sobre impuestos" onPress={() => setTaxModalOpen(true)} style={styles.infoButton}><Ionicons color="#101c2d" name="information" size={18} /></Pressable>
                </View>
              ...
              <View style={styles.field}>
                ...
                <Pressable accessibilityRole="button" onPress={selectImage} style={styles.imageField}>
                  ...
                  <View style={styles.imageCopy}><Text style={styles.imageTitle}>Imagen del servicio</Text><Text style={styles.imageHint}>{imageUri ? 'Cambiar imagen' : 'Cargar imagen'}</Text></View>
              ...
              <View style={styles.field}>
                ...
                <View style={styles.colorGrid}>{AGENDA_COLORS.map((color) => <Pressable key={color} accessibilityLabel={`Color ${color}`} onPress={() => setAgendaColor(color)} style={[styles.colorOption, { backgroundColor: color }, agendaColor === color ? styles.colorSelected : null]}>{agendaColor === color ? <Ionicons color="#fff" name="checkmark" size={18} /> : null}</Pressable>)}</View>
              </View>
      ...
      <CategoryDialog onClose={() => setCategoryModalOpen(false)} onSave={(value) => { setCategory(value); setCategoryModalOpen(false); }} visible={categoryModalOpen} />
      <TaxDialog onClose={() => setTaxModalOpen(false)} onSave={(value) => { setTax(value); setTaxModalOpen(false); }} visible={taxModalOpen} />
    </Modal>
  );
}

function CheckRow({ checked, label, onPress }: { readonly checked: boolean; readonly label: string; readonly onPress: () => void }) {
  return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked }} onPress={onPress} style={styles.checkboxRow}><Ionicons color="#101c2d" name={checked ? 'checkbox' : 'square-outline'} size={25} /><Text style={styles.checkboxLabel}>{label}</Text></Pressable>;
}
...
function PercentageSlider({ label, onChange, value }: { readonly label: string; readonly onChange: (value: number) => void; readonly value: number }) {
  ...
  const setValue = (event: GestureResponderEvent) => { if (trackWidth) onChange(Math.max(0, Math.min(100, Math.round(event.nativeEvent.locationX * 100 / trackWidth)))); };
  return <View><View style={styles.sliderHeading}><Text style={styles.sliderLabel}>{label}</Text><Text style={styles.sliderValue}>{value}%</Text></View><View accessibilityLabel={`${label}: ${value}%`} accessibilityRole="adjustable" onLayout={(event) => setTrackWidth(event.nativeEvent.layout.width)} onMoveShouldSetResponder={() => true} onResponderGrant={setValue} onResponderMove={setValue} onResponderRelease={setValue} onResponderTerminationRequest={() => false} onStartShouldSetResponder={() => true} style={styles.sliderTrack}><View pointerEvents="none" style={[styles.sliderFill, { width: `${value}%` }]} /><View pointerEvents="none" style={[styles.sliderThumb, { left: `${value}%` }]} /></View></View>;
}
...
function Dialog({ children, onClose, visible }: { readonly children: React.ReactNode; readonly onClose: () => void; readonly visible: boolean }) {
  return <Modal animationType="fade" onRequestClose={onClose} transparent visible={visible}><View style={styles.dialogLayer}><Pressable onPress={onClose} style={styles.dialogBackdrop} /><View style={styles.dialog}><Pressable accessibilityLabel="Cerrar" onPress={onClose} style={styles.dialogClose}><Ionicons color="#667080" name="close" size={21} /></Pressable>{children}</View></View></Modal>;
}
...
function CategoryDialog({ onClose, onSave, visible }: { readonly onClose: () => void; readonly onSave: (category: ServiceCategory) => void; readonly visible: boolean }) {
  ...
  const close = () => { setCreating(false); setName(''); setDescription(''); onClose(); };
  const save = () => { if (name.trim()) { onSave({ name: name.trim(), description: description.trim() }); setCreating(false); setName(''); setDescription(''); } };
  return <Dialog onClose={close} visible={visible}>{creating ? <><Text style={styles.dialogTitle}>Añadir categoría de servicio</Text><Text style={styles.dialogCopy}>Añade los detalles de la categoría.</Text><Text style={styles.label}>Nombre</Text><TextInput onChangeText={setName} placeholder="Ej. Cortes" placeholderTextColor="#98a0ab" style={styles.dialogInput} value={name} /><Text style={styles.label}>Descripción</Text><TextInput multiline onChangeText={setDescription} placeholder="Describe esta categoría" placeholderTextColor="#98a0ab" style={[styles.dialogInput, styles.dialogTextArea]} textAlignVertical="top" value={description} /><NavaButton icon="checkmark-outline" label="Guardar cambios" onPress={save} style={styles.dialogButton} variant="primary" /></> : <><View style={styles.dialogIcon}><Ionicons color="#101c2d" name="information" size={28} /></View><Text style={styles.dialogTitle}>Categoría de servicio</Text><Text style={styles.dialogCopy}>Debes crear una categoría y añadir sus detalles antes de asociarla a este servicio.</Text><NavaButton icon="add-outline" label="Crear categoría" onPress={() => setCreating(true)} style={styles.dialogButton} variant="primary" /></>}</Dialog>;
}
...
function TaxDialog({ onClose, onSave, visible }: { readonly onClose: () => void; readonly onSave: (tax: ServiceTax) => void; readonly visible: boolean }) {
  const [creating, setCreating] = useState(false); const [name, setName] = useState(''); const [percentage, setPercentage] = useState(15); const [checkout, setCheckout] = useState(true); const [purchaseEnd, setPurchaseEnd] = useState(false); const [showInfo, setShowInfo] = useState(false);
  const close = () => { setCreating(false); setName(''); setPercentage(15); setCheckout(true); setPurchaseEnd(false); setShowInfo(false); onClose(); };
  const save = () => { if (name.trim()) { onSave({ name: name.trim(), percentage, addAtCheckout: checkout, addAtPurchaseEnd: purchaseEnd }); close(); } };
  return <Dialog onClose={close} visible={visible}>{creating ? <><Text style={styles.dialogTitle}>Añadir impuesto</Text><Text style={styles.label}>Nombre</Text><TextInput onChangeText={setName} placeholder="Ej. IVA" placeholderTextColor="#98a0ab" style={styles.dialogInput} value={name} /><PercentageSlider label="Valor de impuesto" onChange={setPercentage} value={percentage} /><View style={styles.taxOptions}><View style={styles.taxHeader}><Text style={styles.label}>Aplicación del impuesto</Text><Pressable accessibilityLabel="Información sobre impuesto" onPress={() => setShowInfo((current) => !current)} style={styles.infoButton}><Ionicons color="#101c2d" name="information" size={18} /></Pressable></View>{showInfo ? <Text style={styles.taxInfo}>Puedes añadir el impuesto durante el checkout o aplicarlo al finalizar la compra.</Text> : null}<CheckRow checked={checkout} label="Añadir el impuesto en el servicio con checkout" onPress={() => setCheckout((current) => !current)} /><CheckRow checked={purchaseEnd} label="Añadir impuesto al finalizar la compra" onPress={() => setPurchaseEnd((current) => !current)} /></View><NavaButton icon="checkmark-outline" label="Guardar" onPress={save} style={styles.dialogButton} variant="primary" /></> : <><View style={styles.dialogIcon}><Ionicons color="#101c2d" name="information" size={28} /></View><Text style={styles.dialogTitle}>Impuesto del servicio</Text><Text style={styles.dialogCopy}>Crea un impuesto y define cómo debe aplicarse a este servicio.</Text><NavaButton icon="add-outline" label="Crear impuesto" onPress={() => setCreating(true)} style={styles.dialogButton} variant="primary" /></>}</Dialog>;
}
...
const styles = StyleSheet.create({
  ...
  colorSelected: { borderColor: '#101c2d', borderWidth: 3, transform: [{ scale: 1.12 }] },
  content: {
    paddingBottom: 18,
  ...
  sliderFill: { backgroundColor: '#101c2d', borderRadius: 4, bottom: 0, left: 0, position: 'absolute', top: 0 },
  sliderHeading: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 11 },
  sliderLabel: { color: '#101c2d', flex: 1, fontSize: 14, fontWeight: '800' },
  sliderThumb: { backgroundColor: '#fff', borderColor: '#101c2d', borderRadius: 10, borderWidth: 3, height: 20, marginLeft: -10, position: 'absolute', top: -7, width: 20 },
  sliderTrack: { backgroundColor: '#d7e2f8', borderRadius: 4, height: 7 },
  sliderValue: { color: '#101c2d', fontSize: 15, fontWeight: '900' },
  taxHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' },
